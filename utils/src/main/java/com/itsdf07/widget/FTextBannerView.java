...
import com.itsdf07.utils.FDisplayUtils;
import com.itsdf07.utils.R;

import java.util.List;

/**
 * @Description 文案自定义跑马灯
 * @Author itsdf07
 * @Time 2018/11/20
 */

public class FTextBannerView extends RelativeLayout {
    /**
     * @Description 点击监听回调接口
     */
    ...
    @AnimRes
    private int inAnimResId = R.anim.fanim_right_in;
    @AnimRes
    private int outAnimResId = R.anim.fanim_left_out;
    ...
    public FTextBannerView(Context context) {
        this(context, null);
    }

    public FTextBannerView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (typedArray.hasValue(R.styleable.TextBannerViewStyle_setTextSize)) {//设置文字尺寸
            ...
            mTextSize = FDisplayUtils.px2sp(context, mTextSize);
        }
        ...
        if (hasSetDirection) {
            switch (direction) {
                case DIRECTION_BOTTOM_TO_TOP:
                    inAnimResId = R.anim.fanim_bottom_in;
                    outAnimResId = R.anim.fanim_top_out;
                    break;
                case DIRECTION_TOP_TO_BOTTOM:
                    inAnimResId = R.anim.fanim_top_in;
                    outAnimResId = R.anim.fanim_bottom_out;
                    break;
                case DIRECTION_RIGHT_TO_LEFT:
                    inAnimResId = R.anim.fanim_right_in;
                    outAnimResId = R.anim.fanim_left_out;
                    break;
                case DIRECTION_LEFT_TO_RIGHT:
                    inAnimResId = R.anim.fanim_left_in;
                    outAnimResId = R.anim.fanim_right_out;
                    break;
            }
        } else {
            inAnimResId = R.anim.fanim_right_in;
    ...
    public void setDatas(List<String> datas) {
        this.mDatas = datas;
        if (FDisplayUtils.notEmpty(mDatas)) {
    ...
    public void setDatasWithDrawableIcon(List<String> datas, Drawable drawable, int size, int direction) {
        this.mDatas = datas;
        if (FDisplayUtils.isEmpty(mDatas)) {
            return;
        }
        mViewFlipper.removeAllViews();
        for (int i = 0; i < mDatas.size(); i++) {
...
//xml使用
//<com.itsdf07.widget.FTextBannerView
...
//mTvBanner.startViewAnimator();//开始
//mTvBanner.stopViewAnimator();//结束